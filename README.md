...
- **정합성을 확인할 장치가 없습니다.** 카운터와 등록 문서는 같은 트랜잭션에서 갱신되므로 앱을 통한 신청 경로에서는 어긋날 수 없지만, 그 밖의 경로로 생긴 불일치를 감지할 수단이 없습니다. 합계 대조와 번호 구간의 겹침·공백 검사를 주기적으로 돌려 **알리기만 하는** 장치가 있었다면 좋았을 것입니다. 자동으로 카운터를 고치는 방식은 트랜잭션 밖에서 쓰는 순간 동시 증가분을 덮어쓸 수 있어 적절하지 않습니다.
